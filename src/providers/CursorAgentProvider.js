 ...
 * 
 * This provider wraps the Cursor Agent CLI tool for executing AI-powered tasks 
 * with advanced tool calling and workflow automation capabilities.
 */
class CursorAgentProvider extends Provider {
  
  /**
   * Constructor for CursorAgentProvider
   */
  constructor(options = {}) {
    super({
      name: 'CursorAgent',
      ...
      timeout: options.timeout || 180000, // 180 seconds for Cursor Agent
  ...
  /**
   * Generates a prompt for Cursor Agent task execution
   * 
  ...
  async generatePrompt(task, context = {}) {
    ...
    promptParts.push('5. Use appropriate tool calls for file operations and validation');
    promptParts.push('6. Return the implementation code within a code block');
  ...
  /**
   * Parses the output from Cursor Agent CLI execution
   * 
  ...
  async parseOutput(result, context = {}) {
    ...
    try {
      // Cursor Agent can output both code and tool calls
      ...
      const workflowLines = responseText.filter(line => 
      ...
      if (workflowLines.length > 0) {
        output.workflow = workflowLines;
      }
      
    } catch (error) {
      console.warn(`Failed to parse Cursor Agent output: ${error.message}`);
  ...
  /**
   * Simple tool call parser for Cursor Agent output
   * 
  ...
  /**
   * Checks if Cursor Agent CLI is available and healthy
   * 
   * @returns {Promise<boolean>} True if Cursor Agent is healthy
   */
  async checkHealth() {
    try {
      ...
      if (!healthCheck) {
        console.warn('Cursor Agent CLI command not found or not working');
        return false;
      }
      
      // Additional check for Cursor Agent-specific functionality
      try {
        const { exec } = require('child_process');
        await new Promise((resolve, reject) => {
          exec('cursor-agent --list-models', { timeout: 10000 }, (error, stdout, stderr) => {
            if (error) {
              // This is optional, so don't fail if models command doesn't work
              resolve(true);
            } else {
              console.log(`Cursor Agent models available: ${stdout.trim()}`);
              resolve(true);
            }
          });
        });
      ...
      console.warn(`Cursor Agent health check failed: ${error.message}`);
      return false;
    }
  }
  
  /**
   * Builds the CLI command to execute for Cursor Agent
   * 
